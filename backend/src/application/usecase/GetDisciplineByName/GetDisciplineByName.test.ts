...
describe("GetDisciplineByName", () => {
  ...
  it("should throw an error when name is not provided", async () => {
    await expect(getDisciplineByName.execute("")).rejects.toThrow("Discipline name is required")
  })

  it("should throw an error when the discipline does not exist!", async () => {
    await expect(getDisciplineByName.execute("any_name")).rejects.toThrow(`Discipline: "any_name" does not exist!`)
  })
})
