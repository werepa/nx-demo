...
import { disciplineState, disciplineMock } from "../../../tests/mocks/disciplineMock"
...
describe("UpdateDiscipline", () => {
  ...
  it("should successfully update a discipline", async () => {
    ...
    const disciplineUpdated = await updateDiscipline.execute(disciplineState(discipline))
  ...
  it("should throw an error if the discipline does not exist!", async () => {
    const discipline: Discipline = disciplineMock()
    await expect(updateDiscipline.execute(disciplineState(discipline))).rejects.toThrow(
      `Discipline ID:"${discipline.id}" does not exist!`
    )
  })
  ...
  it("should throw an error if the discipline does not have a name", async () => {
    ...
    await expect(updateDiscipline.execute(disciplineState(discipline))).rejects.toThrow("Missing required properties")
  })
  ...
  it("should throw an error if the name already exists in another discipline", async () => {
    ...
    await expect(updateDiscipline.execute(disciplineState(discipline2))).rejects.toThrow(
      `Discipline: "${discipline1.name}" already exists!`
    )
  })
})
