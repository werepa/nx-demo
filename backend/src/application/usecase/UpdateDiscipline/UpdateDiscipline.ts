...
export class UpdateDiscipline {
  ...
  async execute(dto: DisciplineFromPersistence): Promise<Discipline | null> {
    const discipline = Discipline.toDomain(dto)
    await this.validateDisciplineExists(dto.disciplineId)
    await this.validateDisciplineNameUniqueness(
      discipline.name,
  ...
  private async validateDisciplineExists(disciplineId: string): Promise<void> {
    const disciplineExists =
      await this.disciplineRepository.getById(disciplineId)
    if (!disciplineExists) {
      throw new Error(`Discipline ID:"${disciplineId}" does not exist!`)
    }
  }

  private async validateDisciplineNameUniqueness(
    name: string,
    disciplineId: string,
  ): Promise<void> {
    const disciplineNameExists = await this.disciplineRepository.getByName(name)
    if (
      disciplineNameExists &&
      disciplineNameExists.disciplineId !== disciplineId
    ) {
