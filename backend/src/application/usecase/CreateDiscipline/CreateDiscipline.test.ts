...
describe('Usecase => CreateDiscipline', () => {
  ...
  it('should not create a discipline with an existing name', async () => {
    const discipline = disciplineMock();
    const result = await createDiscipline.execute({ name: discipline.name });
    await expect(
      createDiscipline.execute({ name: discipline.name })
    ).rejects.toThrow(`Discipline: "${discipline.name}" already exists!`);
  });
});
