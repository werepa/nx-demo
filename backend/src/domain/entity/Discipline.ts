...
export class Discipline extends Entity<DisciplineProps> {
  ...
  static create(dto: CreateDisciplineInput): Discipline {
    if (!dto.name) {
  ...
  public static toDomain(dto: DisciplineFromPersistence): Discipline {
    ...
    if (missingProperties.length > 0) {
      throw new Error(
        `Discipline - Missing required properties: [${missingProperties.join(", ")}]`,
      )
    }
    ...
    dto.topics
      .filter((topicFromPersistence) => !topicFromPersistence.topicParentId)
      .map((topicFromPersistence) => {
      ...
      dto.topics
        ...
        .map((topicFromPersistence) => {
          const topicParent = discipline.topic({
            topicId: topicFromPersistence.topicParentId ?? "",
          })
          if (topicParent) {
            const topic = Topic.toDomain(topicFromPersistence)
  ...
  topic({ topicId, name }: { topicId?: string; name?: string }): Topic | null {
    ...
    if (name) {
      return this.topics
        .getItems()
        .filter(
          (topic: Topic) => topic.name.toLowerCase() === name.toLowerCase(),
        )[0]
    }
    return null
  }
  ...
  topicsChildren(topicId: string): Topic[] {
    return this.topics
      .getItems()
      .filter((topic: Topic) => topic.topicParentId === topicId)
  }
