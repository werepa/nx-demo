...
describe('DisciplineRepositoryDatabase', () => {
  ...
  describe('Discipline', () => {
    test('should save a discipline', async () => {
      ...
      savedDisciplines[0].topics
        .getItems()[0]
        .setChanged(topic1.updatedAt.value);
      savedDisciplines[0].topics
        .getItems()[1]
        .setChanged(topic2.updatedAt.value);
      savedDisciplines[0].topics
        .getItems()[2]
        .setChanged(topic3.updatedAt.value);
      expect(savedDisciplines[0].topics.getItems()).toEqual([
        topic1,
        topic2,
        topic3,
      ]);
    ...
    test('should update an existing discipline', async () => {
      ...
      discipline
        .topic({ topicId: topic2.topicId })
        ?.updateName('topic 2 changed');
      await disciplineRepository.save(discipline);
      const savedDisciplines = await disciplineRepository.getAll();
      expect(savedDisciplines).toHaveLength(1);
      ...
      expect(savedDisciplines[0].topics.getItems()).toHaveLength(3);
      expect(savedDisciplines[0].topic({ topicId: topic2.topicId })?.name).toBe(
        'topic 2 changed'
      );
    ...
    test('should get a discipline by ID', async () => {
      const discipline = disciplineMock();
      await disciplineRepository.save(discipline);
      const savedDiscipline = await disciplineRepository.getById(
    ...
    test('should return null if discipline does not exist!', async () => {
      const discipline = await disciplineRepository.getById(
        faker.string.uuid()
      );
